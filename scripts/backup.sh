...
if [ -f ".env" ]; then
    tar czf - .env | openssl enc -aes-256-cbc -salt -out "$BACKUP_DIR/env_${TIMESTAMP}.tar.gz.enc"
...
echo ""
echo "To restore .env:"
echo "  openssl enc -aes-256-cbc -d -in $BACKUP_DIR/env_${TIMESTAMP}.tar.gz.enc | tar xz"